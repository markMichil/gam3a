          ...
          <h3 class="ls-top-header">الموردين</h3>
              <div class="panel panel-default">
                  <div class="panel-heading">
                    <h3 class="panel-title">جميع الموردين ( {{count($data)}} )</h3>
                 </div>
...
@endif
          ...
          <a href="{{ url('moward/create') }}" class="btn btn-primary"><i class="fa fa-plus-circle"></i> أضف مورد جديد</a><br/><br/>
              ...
              <table class="table table-bordered  table-bottomless" id="ls-editable-table">
                <thead>
                    <tr>
                     ...
                     <th class="text-center" style='width:10%;font-weight:bold'>تاريخ اخر دفعة</th>
               ...
               <tbody>
                  ...
                  @foreach($data as $key => $row)
                      <tr>
                          ...
                          <td class='text-center'>{{App\Mward::get_last_date($row->id)}}</td>
                          <td class='text-center'>{{$row->total}}</td>
                          <td class='text-center'>{{$row->paid}}</td>
                          <td class='text-center'>{{$row->remain}}</td>
                          <td class='text-center'>
                          	{!! Form::Open(['url'=>'moward/del/'.$row->id]) !!}
                                 <a href="{{ url('moward/edit/'.$row->id) }}" class="btn btn-success"><i class="fa fa-edit"></i> تعديل</a>
